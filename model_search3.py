...
class Cell(nn.Module):
    ...
    def __init__(self, steps, genotypes, C_prev_prev, C_prev, C):
        ...
        # 当genotypes为空，说明还没有learned cell
        # 这样还是不行，因为即使有learned cell，当前search cell也有可能连接到stem节点
...
class Network(nn.Module):
    ...
    def _initialize_alphas(self):
        k = sum(1 for i in range(self._steps) for n in range(2 + 2 * len(self._genotypes) + i))  # 应该把2替换成learned_cell的个数
