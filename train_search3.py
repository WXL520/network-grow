...
from model_search3 import Network, SubNetwork
...
parser.add_argument('--subnet_epochs', type=int, default=200, help='num of training epochs for subnet')
parser.add_argument('--init_channels', type=int, default=16, help='num of init channels')
parser.add_argument('--layers', type=int, default=1, help='total number of layers')
# parser.add_argument('--sublayers', type=int, default=10, help='total number of subnet')
...
parser.add_argument('--max_grow_steps', type=int, default=2, help='max grow steps from a small arch')
...
def main():
    ...
    for i in range(args.max_grow_steps):
        ...
        if i == 0:
            # a = 0
            # genotypes = [Genotype(normal=[('sep_conv_3x3', 0), ('skip_connect', 1), ('sep_conv_3x3', 2), ('sep_conv_5x5', 0)])]
            genotypes = [
                Genotype(normal=[('sep_conv_3x3', 0), ('dil_conv_5x5', 1), ('sep_conv_5x5', 2), ('sep_conv_5x5', 0)])]
            print(f'new genotypes: {genotypes}')
        if i == 1:
            genotypes = [
                Genotype(normal=[('sep_conv_3x3', 0), ('dil_conv_5x5', 1), ('sep_conv_5x5', 2), ('sep_conv_5x5', 0)]),
                Genotype(normal=[('sep_conv_3x3', 2), ('sep_conv_3x3', 0), ('sep_conv_3x3', 3), ('skip_connect', 0)])]
            print(f'new genotypes: {genotypes}')

            # 新得到的genotypes好不好，需要构建model并训练来进行验证，如果好就继续grow？如果不好呢，返回上个step重新搜？
            subnet_acc = train_searched_model(genotypes)
            if subnet_acc > best_acc:
                best_acc = subnet_acc
...
def search_curr_genotype(genotypes, train_queue, valid_queue, criterion):
    # 搜索当前新增加的两个op的类型和连接
    # genotype = Genotype(normal=[('dil_conv_5x5', 0), ('sep_conv_5x5', 1), ('sep_conv_5x5', 2), ('sep_conv_5x5', 0)])
    # 根据上一个step得到的genotypes，构建base model+当前的搜索块
...
def train_searched_model(genotypes):
    model = SubNetwork(args.init_channels, CIFAR_CLASSES, args.layers, genotypes)
    model = model.cuda()
    # logging.info("param size = %fMB", utils.count_parameters_in_MB(model))
