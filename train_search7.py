...
from model_search7 import Network, SubNetwork
...
parser.add_argument('--subnet_epochs', type=int, default=10, help='num of training epochs for subnet')  # 设为20会导致无法产生one-hot，two-hot分布
...
parser.add_argument('--arch_lr_beta', type=float, default=3e-4, help='')
...
def search_curr_genotype(genotypes):
    ...
    if len(genotypes) > 100:
        pretrained_model_dict = torch.load('subnet_weights5.pt')
    ...
    # temp_scheduler = Temp_Scheduler(args.epochs, model.temp, args.temperature, temp_min=args.temperature_min)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # print(f'temperature: {model.temp}')
        ...
        # for i in range(len(model.beta_normal)):
        #     print(F.softmax(model.beta_normal[i], dim=-1))
        print(F.softmax(streng_func(model.alphas_normal), dim=-1))
        print(F.softmax(streng_func2(model.beta_normal0), dim=-1).detach())
        print(F.softmax(streng_func2(model.beta_normal1), dim=-1).detach())
    ...
    torch.save(model.state_dict(), 'supernet_weights5.pt')
...
def train_searched_model(genotypes, inheritance=False):
    ...
    if inheritance:
        # if len(genotypes) > 2:
        pretrained_supernet_weights = torch.load('supernet_weights5.pt')
    ...
    for epoch in range(args.subnet_epochs):
        ...
        if valid_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'subnet_weights5.pt')
