...
from model_search2 import Network
...
def main():
    ...
    criterion = criterion.cuda()  # 这里用不用改？
    # 每一个grow step，这里的network都应该改变吗？用searched model+新的search space代替？
    # 搞个for循环？每次都把上一个network的架构参数还有权重参数作为下一个的输入？
    ...
    architect = Architect(model, args)  # 架构参数的优化？
    ...
    for epoch in range(args.epochs):
        ...
        print('genotype = %s', genotype)  # 存储到文件？
